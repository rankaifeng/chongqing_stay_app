...
export default function ApplicationLetter() {
	...
	const [state, setState] = useState(() => {
		...
		return {
			currentDate: new Date(year, month, day).getTime(),
		};
	});
	const onDeineClick = () => {
		const mData = {
		...
		<View className='application-letter'>
			...
			<Popup
				...
				<View className='popup-btns'>
					<View className='cancel-btn' onClick={() => setTitle(undefined)}>
						取消·Cancel
					</View>
					<View onClick={onOtherDeineClick} className='confirm-btn' style={{ marginLeft: "15px" }}>
						确定·Define
					</View>
				</View>
			</Popup>
